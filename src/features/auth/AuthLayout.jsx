...
export const AuthLayout = ({ children, title, subtitle, showBack, onBack }) => {
  ...
  const shouldAnimate = !hasPlayedInitialAnimation;

  // Отмечаем, что анимация проиграна (эффект сработает после рендера)
  React.useEffect(() => {
  ...
  return (
    <div className="min-h-screen h-screen flex bg-white overflow-hidden">
      {/* Левая часть - фото с диагональным обрезом (скрыта на мобильных) */}
      <div className="hidden lg:block lg:w-1/2 xl:w-[45%] h-full relative shrink-0 bg-neutral-100">
        {/* Изображение концерта */}
        <img 
          src={concertImage} 
          alt="Концерт" 
          className={`absolute inset-0 w-full h-full object-cover ${shouldAnimate ? 'animate-photo-reveal' : ''}`}
          style={{ objectPosition: '0% center' }}
          loading="eager"
        />
        {/* Диагональный обрез - белый треугольник справа */}
        <div 
          className="absolute top-0 bottom-0 -right-[1px] w-[300px] bg-white"
          style={{ clipPath: 'polygon(100% 0, 100% 100%, 0 100%)' }}
        />
      </div>

      {/* Правая часть - форма */}
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 lg:px-12 xl:px-20 relative animate-fade-in">
        {/* Кнопка назад */}
        {showBack && (
          <button 
            onClick={onBack || (() => navigate(-1))}
            className="absolute left-6 top-6 lg:left-12 w-10 h-10 flex items-center justify-center rounded-full hover:bg-neutral-100 transition-all duration-200 hover:scale-105"
          >
